<template>
  <div class="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 relative">
    <header class="px-5 py-4">
      <h2 class="font-semibold text-slate-800 dark:text-slate-100">Supplier Orders <span
          class="text-slate-400 dark:text-slate-500 font-medium">{{ pSupplierOrders.totalResult }}</span></h2>
    </header>
    <div>
      ...
      <div class="overflow-x-auto">
        <table class="table-auto w-full dark:text-slate-300">
          ...
          <tbody class="text-sm divide-y divide-slate-200 dark:divide-slate-700">
            <SupplierOrdersTableItem v-for="order in pSupplierOrders.data" :key="order.id" :order="order"
...
import SupplierOrdersTableItem from './SupplierOrdersTableItem.vue'
import ConfirmDelete from '@/components/ConfirmDelete.vue'

export default {
  name: 'SupplierOrdersTable',
  components: {
  ...
  setup(props, { emit }) {
    ...
    const checkAll = () => {
      selected.value = []
      if (!selectAll.value) {
        const products = pSupplierOrders.value.data;
